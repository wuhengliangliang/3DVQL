...
y_center_num = 16
z_center_num = 16
space_range = [[0, -2.0, -1.0], [10.0, 2.0, 1.0]]  # [[xmin,ymin,zmin],[xmax,ymax,zmax]]
...
class ClipMatcher(nn.Module):
    ...
    def project_imgfeat_to_3dfeat(self, pcd_feat, img_feat):
        ...
        # 按最终约定生成各轴采样点：x->D, y->W, z->H
        x_lin = torch.linspace(space_range[0][0], space_range[1][0], steps=D, device=device)  # x
        y_lin = torch.linspace(space_range[0][1], space_range[1][1], steps=W, device=device)  # y
        z_lin = torch.linspace(space_range[0][2], space_range[1][2], steps=H, device=device)  # z

        # meshgrid 产出维序 [D, H, W]，并按 (x,y,z) 组装
        X, Y, Z = torch.meshgrid(x_lin, y_lin, z_lin, indexing='ij')  # [D,H,W]
        grid_3d = torch.stack([X, Y, Z], dim=-1).view(-1, 3)          # (x,y,z)
        ...
        grid_2d[..., 0] = (uv[:, 0].view(D, H, W) / max(W0 - 1, 1)) * 2 - 1  # u -> width
        grid_2d[..., 1] = (uv[:, 1].view(D, H, W) / max(H0 - 1, 1)) * 2 - 1  # v -> height
    ...
    def forward(self,
        ...
        if fix_backbone:
            ...
            # query_feat3d = self.backbone3d(query_pcd)      #[b, c, d, h, w]
            # clip_feat3d = self.backbone3d(clip_pcd)      # (b t) c d h w
        query_feat3d = self.backbone3d(query_pcd)#.permute(0, 1, 2, 4, 3)  #[b, c, d, h, w]
        clip_feat3d = self.backbone3d(clip_pcd)#.permute(0, 1, 2, 4, 3)      # (b t) c d h w
