...
from evaluation.structures import ResponseTrack
from metrics.utils import PRINT_FORMAT
from metrics.utils import spatio_temporal_mate, interpolated_prec_rec
...
class MATEDetection(object):

    metric_uuid: str = "MATE AP"
    mate_thresholds: np.ndarray = np.array([0.25, 0.5, 0.75, 0.95])
    ...
    def evaluate(self) -> None:
        ...
        self.ap = compute_average_precision_detection(
            self.ground_truth, self.prediction, self.mate_thresholds
        )
    ...
    def get_metrics(self) -> Dict[str, float]:
        self.evaluate()
        metrics = OrderedDict()
        avg_suffix = f"@ Distance_threshold={self.mate_thresholds[0]:.2f}:{self.mate_thresholds[-1]:.2f}"
        if not self.ignore_mate_averaging:
            metrics[PRINT_FORMAT.format(self.metric_uuid, avg_suffix)] = self.average_ap
        for tiou_idx, tiou_thr in enumerate(self.mate_thresholds):
            if tiou_thr not in self.ious_to_report:
                continue
            metrics[
                PRINT_FORMAT.format(self.metric_uuid, f"@ Distance_threshold={tiou_thr:.2f}")
...
def compute_average_precision_detection(
    ground_truth: pd.DataFrame,
    prediction: pd.DataFrame,
    mate_thresholds: Sequence[float] = np.linspace(0.5, 0.95, 10),
) -> np.ndarray:
    ...
    """
    ap = np.zeros(len(mate_thresholds))
    if prediction.empty:
        return ap

    npos = float(len(ground_truth))
    lock_gt = np.ones((len(mate_thresholds), len(ground_truth))) * -1
    # Sort predictions by decreasing score order.
    sort_idx = prediction["score"].values.argsort()[::-1]
    prediction = prediction.loc[sort_idx].reset_index(drop=True)

    # Initialize true positive and false positive vectors.
    tp = np.zeros((len(mate_thresholds), len(prediction)))
    fp = np.zeros((len(mate_thresholds), len(prediction)))
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        tiou_arr = spatio_temporal_mate(
            this_pred["response_track"], this_gt["response_track"].values
        )
        # For MATE (distance), smaller is better -> sort ascending (smallest distance first).
        tiou_sorted_idx = tiou_arr.argsort()
        for tidx, tiou_thr in enumerate(mate_thresholds):
    ...
    for tidx in range(len(mate_thresholds)):
