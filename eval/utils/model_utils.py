...
def positionalencoding4d(d_model, height, width, depth, time, type='sinusoidal'):
    """
    :param d_model: dimension of the model
    :param height: height of the positions
    :param width: width of the positions
    :param depth: depth of the positions
    :param time: time of the positions
    :return: d_model*height*width*depth*time position matrix, shape [H*W*D*T, C]
    """
    if type == 'sinusoidal':
        d_model_interv = int(np.ceil(d_model / 8) * 2)
        if d_model_interv % 2:
            d_model_interv += 1
        inv_freq = 1.0 / (10000 ** (torch.arange(0, d_model_interv, 2).float() / d_model_interv))
        pos_x = torch.arange(height).type(inv_freq.type())
        pos_y = torch.arange(width).type(inv_freq.type())
        pos_z = torch.arange(depth).type(inv_freq.type())
        pos_t = torch.arange(time).type(inv_freq.type())
        sin_inp_x = torch.einsum("i,j->ij", pos_x, inv_freq)
        sin_inp_y = torch.einsum("i,j->ij", pos_y, inv_freq)
        sin_inp_z = torch.einsum("i,j->ij", pos_z, inv_freq)
        ...
        emb_t = get_emb(sin_inp_t)
        emb = torch.zeros(height, width, depth, time, d_model_interv * 4)
        emb[:, :, :, :, : d_model_interv] = emb_x
        emb[:, :, :, :, d_model_interv : 2 * d_model_interv] = emb_y
        emb[:, :, :, :, 2 * d_model_interv : 3 * d_model_interv] = emb_z
        emb[:, :, :, :, 3 * d_model_interv :] = emb_t
        emb = rearrange(emb, 'h w d t c -> (h w d t) c')[:,:d_model]
    elif type == 'zero':
        emb = torch.zeros(height * width * depth * time, d_model)
    return emb
...
def BasicBlock_Conv3D(in_dim, out_dim):
    module = nn.Sequential(
