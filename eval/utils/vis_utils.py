    ...
    for pred_id, pred in enumerate(preds):
        ...
        for i in range(B):
            ...
            for j in range(T):
                ...
                # Build detailed title similar to vis_pred_topk
                cur_prob_value = cur_prob[j].item()
                cur_pred_value = torch.sigmoid(cur_prob_pred[j]).item()
                draw_bbox_values_gt = [x.item() for x in cur_bbox[j]]
                draw_bbox_values = [x.item() for x in cur_bbox_pred[j]]
                title_str = (
                    'Prob: gt {:.3f}:[center({:.1f},{:.1f},{:.1f}),size({:.1f},{:.1f},{:.1f}),rot({:.1f},{:.1f},{:.1f})]\n pred {:.3f}:[center({:.1f},{:.1f},{:.1f}),size({:.1f},{:.1f},{:.1f}),rot({:.1f},{:.1f},{:.1f})]'
                ).format(
                    cur_prob_value,
                    draw_bbox_values_gt[0], draw_bbox_values_gt[1], draw_bbox_values_gt[2],
                    draw_bbox_values_gt[3], draw_bbox_values_gt[4], draw_bbox_values_gt[5],
                    draw_bbox_values_gt[6], draw_bbox_values_gt[7], draw_bbox_values_gt[8],
                    cur_pred_value,
                    draw_bbox_values[0], draw_bbox_values[1], draw_bbox_values[2],
                    draw_bbox_values[3], draw_bbox_values[4], draw_bbox_values[5],
                    draw_bbox_values[6], draw_bbox_values[7], draw_bbox_values[8],
                )
...
def vis_pred_clip_inference(clips, queries, pred, save_path, iter_num):
    ...
    # bbox = pred
    # prob = torch.ones(bbox.shape[0]) * 0.9
    ...
    for i in range(T):
        ...
        if cur_prob.item() > 0.5:
            ...
            # ax[0].add_patch(rect)
        plt.savefig(f"{save_path}/_tmp.jpg")
        plt.close()
        frames.append(cv2.imread(os.path.join(save_path, '_tmp.jpg'))[...,::-1])
    save_name = os.path.join(save_path, '{}_{}.gif'.format(iter_num, i))
    imageio.mimsave(save_name, frames, 'GIF', duration=0.2)
