...
def validate_model_predictions(model_predictions, test_annotations):
    ...
    assert len(video_predictions) == len(video_annotations), f"Number of videos in predictions ({len(video_predictions)}) does not match number of videos in annotations ({len(video_annotations)})"
...
def evaluate(gt_file, pred_file):
    ...
    for pair_name, metrics in pair_metrics.items():
        ...
        metrics = {
            ...
            "MATE AP": metrics["MATE AP                        @ Distance_threshold=0.25:0.95"],
            "MATE AP @ Distance_threshold=0.25": metrics["MATE AP                        @ Distance_threshold=0.25"],
            "stAP": metrics["SpatioTemporal AP              @ IoU=0.05:0.95"],
            "stAP @ IoU=0.05": metrics[
                "SpatioTemporal AP              @ IoU=0.05     "
            ],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt-file", required=False, type=str, default='output/VQLOC/infer_outputs/like_ego4d/_gt.json.gz')
    parser.add_argument("--pred-file", required=False, type=str, default='output/VQLOC/infer_outputs/like_ego4d/_results.json.gz')
