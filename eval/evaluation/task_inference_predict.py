...
class Task:
    ...
    def run(self, model, config, device=None):
        ...
        assert len(ret_bboxes) == self.annots[0]['metadata']['video_end_sec'] - self.annots[0]['metadata']['video_start_sec'] + 1, f"Expected {self.annots[0]['metadata']['video_end_sec'] - self.annots[0]['metadata']['video_start_sec'] + 1} bboxes, but got {len(ret_bboxes)}, batch_id: {self.batch_id}, scene_id: {self.scene_id}, clip_id: {self.clip_id}"
    ...
    def prepare_vis_lists(self, img: Image.Image, calib_file: str, frame_info: Dict[str, Any], point_size: float = 0.5) -> Tuple[Dict[str, float], np.ndarray, np.ndarray]:
        ...
        def to_vec3(v):
            ...
            if a.size < 3:
                ...
                return b
            return a[:3]
        if frame_info.get('rotation3D') is None or frame_info.get('center3D') is None or frame_info.get('size3D') is None:
            cen = to_vec3({'x': frame_info.get('x'), 'y': frame_info.get('y'), 'z': frame_info.get('z')})
            siz = to_vec3({'x': frame_info.get('l'), 'y': frame_info.get('w'), 'z': frame_info.get('h')})
            rot = to_vec3({'x': frame_info.get('roll'), 'y': frame_info.get('pitch'), 'z': frame_info.get('yaw')})
    ...
    def _bbox_from_contour(self, frame_info: Optional[Dict[str, Any]]):
        ...
        """
        if frame_info is None:
            bbox = BoundingBox(center=[float(0.), float(0.), float(0.)],
                           size=[0.02, 0.02, 0.02], orientation=Quaternion(axis=[0, 0, 1], radians=0.))
            setattr(bbox, 'rot', [0., 0., 0.])
            return bbox
        ...
        if frame_info.get('rotation3D') is None or frame_info.get('center3D') is None or frame_info.get('size3D') is None:
            cen = to_vec3(dict(x=frame_info.get('x'), y=frame_info.get('y'), z=frame_info.get('z')))
            siz = to_vec3(dict(x=frame_info.get('l'), y=frame_info.get('w'), z=frame_info.get('h')))
            rot = to_vec3(dict(x=frame_info.get('roll'), y=frame_info.get('pitch'), z=frame_info.get('yaw')))
    ...
    def _get_query_(self, comp_template: Dict[str, Any]):
        ...
        if self.config.dataset.get('query_square', False):
...
def inference_video(config, model, data, save_path, device):
    ...
    for i in range(inference_time):
        ...
        clips_origin, clips_i, clips_p, clips_pcd_b = load_clip(config, clips_img, clips_pcd, clips_pcd_bbox, frame_idx)    # clips_i: [N,3,H,W]
        clips_origin = clips_origin[:num_frames]

        # rearrange到 [b,t,c,h,w]
        clips_i = clips_i[:num_frames]
        clips_p = clips_p[:num_frames]
        clips_pcd_b = clips_pcd_b[:num_frames]
        clips_i = rearrange(clips_i, '(b t) c h w -> b t c h w', b=batch_size_inference, t=clip_num_frames)
        clips_p = rearrange(clips_p, '(b t) n c -> b t n c', b=batch_size_inference, t=clip_num_frames)
        # clips_pcd_bbox = rearrange(clips_pcd_bbox, '(b t) c -> b t c', b=batch_size_inference, t=clip_num_frames)
    ...
    # 截断到原始长度
    ret_bboxes = ret_bboxes[:T]
    ret_scores = ret_scores[:T]
