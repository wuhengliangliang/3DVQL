...
def load_annotations(data_path) -> List[Dict[str, Any]]:
    ...
    for batch_id in batch_ids:
        ...
        for scene_id in tqdm(scene_ids, desc='[%6s]Loading annos' % f'batch{batch_id}'):
            ...
            clip_start_valid_idx = []; clip_end_valid_idx = []
            last_valid = False
            for idx, frame_anno in enumerate(search_tracklet_anno):
                real_idx = idx + 1
                if frame_anno.get('contour') is not None and last_valid is False: # 上跳沿
                    clip_start_valid_idx.append(real_idx)
                    last_valid = True
                elif frame_anno.get('contour') is None and last_valid is True: # 下跳沿
                    clip_end_valid_idx.append(real_idx - 1)
                    last_valid = False
            if last_valid is True:
                clip_end_valid_idx.append(len(search_tracklet_anno))

            clip_start_idx = [1]; clip_end_idx = []
            for idx in clip_end_valid_idx[:-1]:
                clip_start_idx.append(idx + 1)
            for idx in clip_end_valid_idx:
                clip_end_idx.append(idx)
            ...
            for clip_idx in range(len(clip_start_valid_idx)):
                ...
                video_end_frame = end_idx + 1 # 补充模板帧
                ...
                for idx in range(clip_start_valid_idx[clip_idx], clip_end_valid_idx[clip_idx] + 1):
                    ...
                    l, w, h = frame_anno['contour']['size3D']['x'], frame_anno['contour']['size3D']['y'], frame_anno['contour']['size3D']['z']
                ...
                # 补充模板帧
                response_track.append({'frame_number' : video_end_frame,
                ...
                query_sets = {
                    "Q0": {
                        "is_valid": True,
                        "query_frame": video_end_frame,
                        "visual_crop": {
                            "frame_number": video_end_frame,
                            "x" : template_tracklet_anno[0]['contour']['center3D']['x'],
                            "y" : template_tracklet_anno[0]['contour']['center3D']['y'],
                            "z" : template_tracklet_anno[0]['contour']['center3D']['z'],
                            "w" : template_tracklet_anno[0]['contour']['size3D']['x'],
