...
def get_dataset(config, split='train'):
    ...
    clip_params = {
        ...
        # 与之前的实现保持一致，补齐这两个键
        'frame_npts': config.dataset.frame_npts,
        'preload_offset': config.dataset.preload_offset,
    }
    ...
    if dataset_name == 'ego4d_vq2d':
        dataset = QueryVideoDataset(
            ...
            # 路径参数保持与“之前代码”一致
            data_dir=os.path.join(config.dataset.root, 'datav2'),
            clip_dir=os.path.join(config.dataset.root, 'datav2/clips'),
            meta_dir=os.path.join(config.dataset.root, 'datav2'),
            video_dir=os.path.join(config.dataset.root, 'datav2/v2/full_scale'),
        )
        ...
        dataset = EgoTracksDataset(
            ...
            data_dir=os.path.join(config.dataset.root, 'datav2'),
            clip_dir=os.path.join(config.dataset.root, 'datav2/clips'),
            meta_dir=os.path.join(config.dataset.root, 'egotrack'),
            video_dir=os.path.join(config.dataset.root, 'datav2/v2/full_scale'),
        )
    elif dataset_name == 'VQLOC':
        ...
        dataset = VQLOC(
            ...
            clip_params=clip_params,
            split=split,
            data_dir=os.path.join(config.dataset.root),
        )
    return dataset
